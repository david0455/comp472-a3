...
from NB_BOW import toDataFrame, feqTotalTweets, freq, filterVocab
...
class NB_Classifier:
    ...
    def getF1(self, test_set, precision, recall):
        return (2 * precision[0] * recall[0] / (precision[0] + recall[0])), \
    ...
    def fit_OV(self, train_set):
        ...
        for h in self.priors:
            ...
            for f in self.vocabulary:
                ...
                if h == "yes":
                    self.likelihood_h0[word_f] = log10((count_c + smoothing) / (
                                tot_num_word_c + smoothing * self.vocabulary.size))  # likelihood h0 <-- {word: probability}
                elif h == "no":
                    self.likelihood_h1[word_f] = log10((count_c + smoothing) / (
                                tot_num_word_c + smoothing * self.vocabulary.size))  # likelihood h1 <-- {word: probability}
    ...
    def fit_FV(self, train_set):
        ...
        for h in self.priors:
            num_tweet_c = (train_set['q1_label'] == h).sum()  # Number of tweets/instance in class c
            df_c = train_set[(train_set['q1_label'] == h)]  # All instances of class c
            num_word_c = toDataFrame(
                filterVocab(feqTotalTweets(df_c.iloc[:, 1])))  # Get all tweets/instances of class c
            tot_num_word_c = num_word_c[0].sum()  # Total number of words in class c
    ...
    def predict(self, test_set):
        df = copy.deepcopy(test_set)
        for f in df.to_numpy():  # Numpy array of all instances in test_set
            ...
            for h in self.priors:
                ...
                for word in headers:
                    ...
                    if voc in self.vocabulary:  # If the word is in the list(BOW) of the vocabulary(from
                        # training_set), add to the score
                        if h == "yes":
                ...
                self.score[
                    h] = score  # When we looped through all the words in the tweet, store the score to its class
            result_class = max(self.score.items(),
                               key=op.itemgetter(1))  # Get the argmax between the score of each class
            self.final_result.append(result_class)  # Append final result in list

        # Put results in dataframe
        most_likely_class, final_score = zip(*self.final_result)
        df['predicted_class'] = most_likely_class
        df['score'] = np.array(final_score).round(2)

        self.accuracy = self.getAccuracy(df)
        self.precision = self.getPrecision(df)  # precision(yes, no)  ->  precision[0] = yes,  precision[1] = no
        self.recall = self.getRecall(df)  # recall(yes, no)     ->  recall[0]    = yes,  recall[1]    = no
        self.f1 = self.getF1(df, self.precision,
                             self.recall)  # f1(yes, no)         ->  f1[0]        = yes,  f1[1]        = no
